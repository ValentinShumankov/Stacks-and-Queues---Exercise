...
namespace _11._Key_Revolver
{
    class Program
    {
        static void Main(string [ ] args)
        {
            ...
            Queue<int> bullets =new Queue<int>( Console.ReadLine().Split().Select(int.Parse).ToArray().Reverse());
            ...
            {
                if ( reloadCycle == barrelSize )
                {
                    Console.WriteLine( "Reloading!" );
                    reloadCycle = 0;
                }
                int curBullet = bullets.Peek();
            ...
            if ( reloadCycle == barrelSize && bullets.Any() )
            {
                Console.WriteLine( "Reloading!" );
            }
            ...
            {
                var finalLoot = ValueOfPrice - spendBullets * bulletPrice;
                Console.WriteLine( $"{bullets.Count} bullets left. Earned ${finalLoot}" );
            }
        }
    }
}
...
Our hero posesses a special weapon though, called the Key Revolver, with special bullets.
